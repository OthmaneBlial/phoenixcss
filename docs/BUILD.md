...
The build cleans `dist/` once, compiles both Sass entries, and minifies them through PostCSS/cssnano with source maps disabled. It does not install a runtime dependency into a consumer page. `npm run lint` checks Sass/CSS/HTML/JavaScript formatting and validates the source HTML with HTML-validate. `npm test` rebuilds CSS and the site, checks CSS structure, contrast, local documentation links, copyable snippets, staged references, package contents and installation, and runs the CSS contract tests. The [core fixture](../examples/core.html) and [full fixture](../examples/themed.html) exercise the separate outputs.
