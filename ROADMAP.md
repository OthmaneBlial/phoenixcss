...
- [x] **Suivi :** correctifs Sass compilés et convention documentée ; grille réelle mesurée de 320 à 1440 px dans Chrome. La [CI du commit `694d523`](https://github.com/OthmaneBlial/phoenixcss/actions/runs/35447229930) a validé dans Chromium, Firefox et WebKit les colonnes `sm/md/lg/xl`, les onze offsets et les utilitaires `display` juste avant et à chaque seuil. La landing réelle n'a pas débordé à 320, 600, 768, 992 ou 1200 px ; ses cinq [captures complètes](assets/screenshots/README.md) ont été relues. Navigation et sidebar restent dans le flux sans masquer le contenu dans les exemples contrôlés. Le comportement au lecteur d'écran relève de 4.2.
...
- [ ] **Suivi :** palette et contraste des paires principales, dont états actifs et bordure d'erreur, mesurés dans le CSS compilé ; focus clavier et select contrôlés dans Safari. Le formulaire montre une erreur textuelle près du champ invalide, puis un état valide local dans Chrome. Axe-core 4.13.0 n'a remonté aucune violation WCAG A/AA sur huit états dans les trois moteurs [CI du commit `1b4d152`](https://github.com/OthmaneBlial/phoenixcss/actions/runs/35446956140), après correction d'un défaut de contraste sérieux dans le pied de page docs. Le focus et l'activation d'une case à cocher ont aussi été vérifiés au clavier dans Chrome sur la palette slate. Contrôle manuel de tous les états, zoom exact et lecteur d'écran restent à faire.
...
- [x] **Suivi :** classes structurelles préfixées, conflit `.align-baseline` levé et fixture hôte inspectée dans Safari/Chrome ; les classes hôtes `.sidebar` et `.footer` gardent leurs styles tandis que `.phx-footer` prend les siens. Test de la fixture à 320, 768 et 1440 px réussi dans Chromium, Firefox et WebKit [en CI](https://github.com/OthmaneBlial/phoenixcss/actions/runs/35446956140). Deux [captures réelles](assets/screenshots/README.md) montrent le même HTML avec et sans la feuille. Le contrôle de structure CSS rejette des déclarations contradictoires ; la table de migration est publiée. Les composants/utilitaires génériques restent volontairement non préfixés et leur risque est documenté.
...
- [x] **Suivi :** rôles CSS sémantiques et palette claire slate documentés ; surcharge copiable sur la feuille complète. Le [run `14466bb`](https://github.com/OthmaneBlial/phoenixcss/actions/runs/35447876286) a passé 27 tests navigateur dans Chromium, Firefox et WebKit : article, formulaire et navigation/cartes/boutons gardent le même DOM et leur texte quand seule la feuille slate est désactivée ; surface, lien, bouton, navigation, bordure et chevron de select changent de couleur. Les deux [captures complètes](assets/screenshots/README.md) ont été relues. Le script de contraste vérifie les paires normales, hover, active, focus et erreur des deux palettes ; axe n'a détecté aucune violation WCAG A/AA sur la page slate dans les trois moteurs. Aucun thème sombre ni commutateur n'est annoncé. La validation au lecteur d'écran reste en 4.2.
...
- **Validation :** charger les deux palettes sur le même DOM, vérifier les styles calculés, les contrastes et les captures des contenus, formulaires et composants exposés ; contrôler les couleurs fixes restantes.
...
- [ ] **Suivi :** trois pages exécutables ajoutées au site assemblé : guide avec bascule réelle du CSS, formulaire slate avec validation locale et landing page responsive. Rendu et interactions contrôlés dans Chrome, landing page également vue dans Safari et à fort zoom. Quinze captures non modifiées issues de quatre runs Chromium CI sont versionnées avec [provenance](assets/screenshots/README.md), dont cinq largeurs de landing et les deux palettes sur un DOM identique relues. Elles devront être revues ou reprises sur le tag final ; lecteur d'écran et matrice manuelle restent à faire.
...
- [x] **Suivi :** `npm test` construit le CSS et le site, contrôle taille, structure, contraste, références et contenu/installabilité du paquet, puis exécute six tests Node. Les tests rejettent des mutations de media query, sélecteur core, déclaration et budget ; une substitution locale du snippet vers `missing.css` a fait échouer son test, puis a été restaurée. La PR [#1](https://github.com/OthmaneBlial/phoenixcss/pull/1) a prouvé que l'absence de CSS dans le tarball fait échouer la CI ([run](https://github.com/OthmaneBlial/phoenixcss/actions/runs/35445239346)), puis que la correction et les 18 parcours navigateur passent ([run](https://github.com/OthmaneBlial/phoenixcss/actions/runs/35445340179)). `npm run lint` vérifie le format Sass/CSS/HTML/JS et les règles HTML recommandées. La validation manuelle de rendu reste en 4.2.
...
- [ ] **Suivi :** rendu réel inspecté avec Safari 26.6 et Chrome 153 sur macOS 26.6 ; largeurs exactes 320 à 1440 px mesurées dans Chrome et le formulaire slate a été inspecté dans Safari après la séquence native Actual Size puis quatre Zoom In. Les 27 tests Playwright ont réussi sur Ubuntu 24.04.5 LTS avec Chromium 153, Firefox 155 et WebKit 26.6 pour le commit [`14466bb`](https://github.com/OthmaneBlial/phoenixcss/actions/runs/35447876286), incluant offsets, mouvement réduit, cohabitation avec un site hôte, comparaison de deux palettes et axe-core sans violation WCAG A/AA détectée sur huit états. Un [rapport daté](docs/VALIDATION.md) et quinze captures réelles séparent ces observations des limites. La valeur exacte de 200 %, le lecteur d'écran, la matrice clavier complète, le contrôle manuel du mouvement réduit et la revue contre le build final restent à faire.
...
- **Fichiers/parties :** **package.json**, nouveau **CHANGELOG.md**, **docs/MIGRATION.md**, **docs/RELEASE_CHECKLIST.md**, **.github/workflows/release.yml**, **README.md**.
...
- [ ] **Suivi :** GitHub Pages du dépôt projet est configuré le 19 septembre 2026 en mode workflow, avec URL prévue `https://othmaneblial.github.io/phoenixcss/`. Le dossier autonome a aussi été poussé dans le dépôt Pages personnel `OthmaneBlial/OthmaneBlial.github.io` au commit [`57e194c`](https://github.com/OthmaneBlial/OthmaneBlial.github.io/commit/57e194c4ed2c0a5eaf6ec7fa803b4f37732b6810) sur `master`, et l'API Pages indique `built`. La route publique répond encore HTTP 404 après rebuild forcé ; aucun lien public fonctionnel n'est annoncé tant que cette propagation n'est pas vérifiée.
...
- [ ] **Suivi :** description About remplacée par la promesse réelle et cinq topics vérifiés via l'API GitHub. Badge CI et portée explicités ; capture desktop et avant/après réels ajoutés au README, quinze captures sourcées au dépôt. Page GitHub rendue et actualisée inspectée dans Chrome : images et lien `ONBOARDING_REVIEW.md` chargés, dernier commit `67f2a51` visible, badge vert et guide de thème reliant aux deux PNG. Métadonnées Open Graph textuelles et image réelle préparées pour le site. URL de site, badge release, liens définitifs et aperçu social public attendent leur publication réelle.
